...
def _last_relevant(output, length1, length2):
    """Finds the output at the end of each input"""
    batch_size = int(output.get_shape()[0])  #20
    max_length = int(output.get_shape()[1])  #1152
    out_size = int(output.get_shape()[2])    #80

    index = tf.range(0, batch_size) * max_length + (length1*length2 - 1)
    flat = tf.reshape(output, [-1, out_size])
    relevant = tf.gather(flat, index)

    return relevant    #20, 80
...
class DMN_PLUS(object):
    ...
    def load_data(self, debug=False):
        ...
        if self.config.train_mode:
            self.train, self.valid, self.word_embedding, self.max_q_len, self.max_input_len,self.vocab_size = snli_input.load_snli(self.config, split_sentences=False)
        else:
            self.test, self.word_embedding, self.max_q_len, self.max_input_len, self.vocab_size = snli_input.load_snli(self.config, split_sentences=False)
    ...
    def get_question_representation(self, embeddings):
        ...
        inputs = tf.nn.embedding_lookup(embeddings, self.question_placeholder)
        # use encoding to get sentence representation
        # inputs = tf.reduce_sum(inputs * self.encoding, 2)

        inputs = tf.split(1, self.max_q_len, inputs)
    ...
    def get_attention(self, q_vec, prev_memory, fact_vec):
        ...
        with tf.variable_scope("attention", reuse=True, initializer=_xavier_weight_init()):
            ...
            features = [fact_vec*q_vec, fact_vec*prev_memory, q_vec*prev_memory, tf.abs(fact_vec - q_vec), tf.abs(fact_vec - prev_memory), tf.abs(q_vec - prev_memory)]

            feature_vec = tf.concat(1, features)  #(,480)
    ...
    def generate_episode(self, memory, q_vec, fact_vecs):
        ...
        # here attentions vector has len(fv) * len(fv) length
        attentions = []
        for q in q_vec:
            attentions += [tf.squeeze(self.get_attention(q, memory, fv), squeeze_dims=[1]) for fv in fact_vecs]
            #print(len(attentions))  48*24 = 1152
        attentions = tf.transpose(tf.pack(attentions))
        self.attentions.append(attentions)

        softs = tf.nn.softmax(attentions)

        softs = tf.split(1, self.max_input_len*self.max_q_len, softs)
        #print(len(softs))  1152
        ...
        # use attention gru
        for i, qv in enumerate(q_vec):
            for j, fv in enumerate(fact_vecs):
                h = self._attention_GRU_step(fv+qv, h, softs[i*len(fact_vecs)+j])
                gru_outputs.append(h)

        # extract gru outputs at proper index according to input_lens
        gru_outputs = tf.pack(gru_outputs)
        gru_outputs = tf.transpose(gru_outputs, perm=[1,0,2]) #(20, 1152, 80)
  
        episode = _last_relevant(gru_outputs, self.input_len_placeholder, self.question_len_placeholder)
    ...
    def add_answer_module(self, rnn_output):
        ...
        with tf.variable_scope("answer"):
            ...
            U = tf.get_variable("U", (self.config.embed_size, 3))
            b_p = tf.get_variable("bias_p", (3,))
    ...
    def inference(self):
        ...
        with tf.variable_scope("memory", initializer=_xavier_weight_init()):
            ...
            prev_memory = tf.get_variable("memory", (self.config.batch_size,self.config.embed_size))
            ...
            for i in range(self.config.num_hops):
                ...
                Wt = tf.get_variable("W_t"+ str(i), (160, self.config.hidden_size))
